...
/**
 * Обобщённые формулировки причин, когда одна и та же причина блокирует все места,
 * но с разными данными.
 *
 * Сообщение конкретного конфликта содержит числа именно того места, для которого
 * оно построено («занято 2 из 2»). Показать такое сообщение как общее — значит
 * выдать достоверное на вид, но неверное для части мест число. Поэтому для
 * корневой причины берётся формулировка без данных места, а конкретика остаётся
 * в карточках вариантов.
 */
export const sharedConflictSummary: Partial<Record<ConflictCode, string>> = {
  location_full: 'Свободных мест нет ни в одном месте реестра.',
  humidity_below_minimum:
    'Ни в одном месте влажность не достигает минимума, требуемого заявкой.',
  anxiety_above_limit:
    'Все места принимают привидений с меньшей тревожностью, чем указана в заявке.',
  location_closed: 'Все места реестра закрыты для новых жильцов.',
}
